...
|---|------------|------|--------------:|--------------:|---------------:|----------|------|
| 1 | official_025 | `0.7*018 + 0.3*004` logits ensemble | 0.9439 | **0.9476** | +0.0037 | 2x ConvNeXt | 当前平台第一 |
| 2 | official_024 | official_018 top-3 checkpoint averaging | 0.9423 | **0.9464** | +0.0041 | 1x ConvNeXt | 当前最强 1x |
| 3 | official_031 | official_018 original + horizontal flip TTA | 0.9431 | **0.9459** | +0.0028 | 2x TTA | 略高于 013，但成本翻倍 |
| 4 | official_013 | `0.7*009 + 0.3*004` logits ensemble | 0.9524 | **0.9456** | -0.0068 | 2x ConvNeXt | 旧平台第一，仍是强基线 |
| 5 | official_030 | official_013 logits + cloudy `-0.5` class bias | 0.9541 | **0.9456** | -0.0085 | 2x ConvNeXt | 平台等于 013，class bias 无收益 |
| 6 | official_018 | 224 warm-start from `exp_044`, EMA | 0.9423 | **0.9446** | +0.0023 | 1x ConvNeXt | 024/025 的核心分支 |
| 7 | official_012 | `0.8*009 + 0.2*004` logits ensemble | 0.9499 | **0.9424** | -0.0075 | 2x ConvNeXt | ensemble 有效，但低于 013/025 |
| 8 | official_023 | 288 ConvNeXt-Tiny | 0.9461 | 0.9420 | -0.0041 | 1x ConvNeXt | 分辨率增大未超过 224 averaging |
| 9 | official_010 | official_009 top-3 checkpoint averaging | 0.9412 | 0.9387 | -0.0025 | 1x ConvNeXt | averaging 在 009 上也有平台小收益 |
| 10 | official_009 | 224 warm-start from `exp_054`, seed=42 | 0.9412 | 0.9373 | -0.0039 | 1x ConvNeXt | 旧 warm-start 基线 |
| 11 | official_019 | 256 warm-start from `exp_054` | 0.9540 | 0.9359 | -0.0181 | 1x ConvNeXt | 本地高分不迁移 |
| 12 | official_004 | official_002 top-3 checkpoint averaging | 0.9315 | 0.9315 | 0.0000 | 1x ConvNeXt | 单独不强，但与 009/018 互补 |
| 13 | official_014 | warm-start seed=7 | 0.9385 | 0.9283 | -0.0102 | 1x ConvNeXt | seed=7 不如 018/024 |
| 14 | official_011 | official_007 top-3 checkpoint averaging | 0.9507 | 0.9280 | -0.0227 | 1x ConvNeXt | holdout 明显高估 |
| 15 | official_022 | progressive resize 256->320 | 0.9572 | 0.9253 | -0.0319 | 1x ConvNeXt | 本地最高但平台翻车 |
| 16 | official_007 | 224 ImageNet init, EMA | 0.9456 | 0.9252 | -0.0204 | 1x ConvNeXt | 非 warm-start 平台不稳 |
| 17 | official_008 | 320 ImageNet init, EMA | 0.9399 | 0.9196 | -0.0203 | 1x ConvNeXt | 320 未带来平台收益 |
| 18 | official_005 | 320, wd=5e-4, EMA | 0.9142 | 0.9188 | +0.0046 | 1x ConvNeXt | 分数低，不作主线 |
| 19 | official_003 | 320, standard aug, EMA | 0.9246 | 0.9158 | -0.0088 | 1x ConvNeXt | 标准增强不稳 |
| 20 | official_001 | 320 baseline, EMA | 0.9376 | 0.9147 | -0.0229 | 1x ConvNeXt | 平台弱 |
| 21 | official_002 | 320 baseline, no EMA | 0.9287 | 0.9059 | -0.0228 | 1x ConvNeXt | 不作主线 |
| 22 | official_006 | 320 low-wd, no EMA | 0.9291 | 0.9034 | -0.0257 | 1x ConvNeXt | 不作主线 |

## 平台待测优先级

| 提交优先级 | Experiment | 配置 | Local holdout | 推理成本 | 统一权重文件 | 判断 |
|------------|------------|------|--------------:|----------|--------------|------|
| P0 | official_035 | `0.7*official_028 + 0.3*official_004` | 0.9439 | 2x ConvNeXt | `outputs/official_035/official_035_best_model.pth` | 可测；025 平台第一后，验证 028 分支替换 018 是否仍有小收益 |
| P1 | official_028 | official_018 dense/SWA-style averaging branch | 0.9423 | 1x ConvNeXt | `outputs/official_028/official_028_best_model.pth` | 可测；024 已证明 averaging 平台有效，但本地没有超过 024 |
...
| Skip | official_033 | 3-fold ConvNeXt-Tiny 224 logits ensemble | 0.9224 | 3x ConvNeXt | `outputs/official_033/official_033_best_model.pth` | 本地显著低于主线，不建议平台测 |
| Skip | official_036 | `0.7*official_033 + 0.3*official_004` | 0.9303 | 4x ConvNeXt | `outputs/official_036/official_036_best_model.pth` | 033+004 未恢复，不建议平台测 |
...
| 10 | official_025 | 018/004 logits ensemble | 0.9439 | **0.9476** | 当前平台第一 |
| 11 | official_035 | 028/004 logits ensemble | 0.9439 | pending | 新增可测候选 |
...
| 16 | official_028 | official_018 dense/SWA-style averaging | 0.9423 | pending | 新增 1x 可测候选 |
...
| 2 | official_018 | 224 warm-start EMA | 0.9423 | 0.9446 | 单模型核心基线 |
| 3 | official_023 | 288 ConvNeXt-Tiny | 0.9461 | 0.9420 | 分辨率更大但平台不占优 |
| 4 | official_010 | official_009 top-3 averaging | 0.9412 | 0.9387 | averaging 有小收益，但低于 018/024 |
| 5 | official_028 | dense/SWA-style averaging | 0.9423 | pending | 可测，但本地没有超过 024 |

## 子榜 B — Ensemble / TTA / Calibration

| # | Experiment | 组成 | Local holdout | Official test | 结论 |
|---|------------|------|--------------:|--------------:|------|
| 1 | official_025 | `0.7*018 + 0.3*004` | 0.9439 | **0.9476** | 当前平台第一，004 与 018 互补成立 |
| 2 | official_031 | official_018 + hflip TTA | 0.9431 | 0.9459 | 有正收益，但 2x TTA 低于 025 |
| 3 | official_013 | `0.7*009 + 0.3*004` | 0.9524 | 0.9456 | 旧平台第一，仍是强基线 |
| 4 | official_030 | official_013 + class bias | 0.9541 | 0.9456 | holdout bias 过拟合，平台无收益 |
| 5 | official_012 | `0.8*009 + 0.2*004` | 0.9499 | 0.9424 | ensemble 有效但权重不如 013/025 |
| 6 | official_035 | `0.7*028 + 0.3*004` | 0.9439 | pending | 可作为 025 的 028 分支对照 |
...
1. **平台正式分数第一是 `official_025`**：0.9476，成本是 2x ConvNeXt。
2. **最强 1x 是 `official_024`**：0.9464，说明 018 top-3 checkpoint averaging 在平台上确实有效。
3. **`official_030` 证明 holdout class-bias 过拟合**：本地 0.9541，但平台等于 013，没有真实增益。
4. **`official_031` 有小正收益但性价比一般**：2x TTA 得到 0.9459，低于 1x 的 024 和 2x 的 025。
5. **3-fold 路线当前不成立**：`official_033` 本地 0.9224，`official_036` 本地 0.9303，不建议扩展到 5-fold。
6. **新增平台测试只剩很窄的验证价值**：优先级是 `official_035`，其次 `official_028`；不建议再增加大规模训练实验。
